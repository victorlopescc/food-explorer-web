...
export function Food({ data, isAdmin, isFavorite, updateFavorite, handleDetails, user_id, ...rest }) {
    ...
    const handleFavorite = async () => {
        try {
            if (isFavorite) updateFavorite(true, data.id)
            else updateFavorite(false, data.id)
        } catch (error) {
            console.error(error)
        }
    }
        ...
        <Container {...rest} isAdmin={isAdmin}>
            {
                isAdmin
                    ...
                    : (<FiHeart size={"2.4rem"} fill={isFavorite ? theme.COLORS.GRAY_200 : undefined} onClick={handleFavorite} />)
            }
            <img src={`${api.defaults.baseURL}/files/${data.image}`} onClick={() => handleDetails(data.id)} />
            <Title>
                <h2>{data.name}</h2>
                <RxCaretRight size={isDesktop ? "2.4rem" : "1.4rem"} onClick={() => handleDetails(data.id)} />
            </Title>
            {
